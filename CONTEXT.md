...
**Duelist**:
The light, quick enemy that owns the space **above the player** from Wave 35, taking up one firing position after another against the player. It enters from the top like most, then never descends into the player's zone and never flees, so a Wave clears only once it is dead. Its life is a single repeating loop: **fly to a Spot, fire the instant it arrives, hold a moment, choose a new Spot.** It **keeps its nose pointed at the player at all times** — the first enemy drawn rotated — and its shots leave along that nose, so the facing is an honest read on where the next bullet goes rather than decoration. The distance it keeps is protection from **ramming**, not from the gun: a Spot can sit directly above the player, and out-timing the loop to line up beneath it is exactly how it dies. Drifting out of its line fails (the nose tracks); a committed break outruns the nose and the later shots trail. At most **two** are ever live, opening on opposite sides so the player is caught in a crossfire and can only line up under one at a time. See ADR-0024. (`fighter3`)
_Avoid_: Gunner (a Gunner descends and its burst is aimed once down a fixed line — the Duelist holds station and its burst tracks), Bomber (the Bomber closes on the player and detonates; the Duelist never closes and never rams), Mini-boss / capstone (a normal-budget pick, not a Wave anchor, and it strafes nothing — it repositions), sniper (it fires a tracking burst, not one heavy round), chasing / following / tracking the player (it never does — it commits to a Spot and flies there; only its *nose* tracks).

**Spot** (Duelist Spot):
The single position a Duelist is flying to or sitting on. Chosen from where the player is **at the instant of choosing** — an angle on an arc around them, a set distance out, pulled into the band above them — and then **committed**: the Duelist flies to the Spot it picked, not to where the player has since gone. That commitment is the whole feel of the enemy. It is why the move reads as a decision rather than a tether, and it is the player's counterplay: move right after it commits and it spends the beat out of position. Each new Spot steps a little further around the arc than the last, reversing at the ends, so a Duelist paces across the player rather than circling.
_Avoid_: anchor / tether (the Spot is frozen, not followed), waypoint (implies a route — there is only ever the next one), orbit (reserve for the Drone and the Warden's Shield, which continuously circle a body).
