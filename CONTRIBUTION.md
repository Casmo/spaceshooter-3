...
| ![Lode](public/assets/SpaceShooter/Enemies/Asteroids-Sheet.png) | **Lode** | Wave 15 | Golden boss-HP rock that drifts slowly across the top every 3rd wave, dripping Stars. Kill it inside its ~16s window for a 10+ Star shower; it never shoots, survives a ram, and always leaves. |
...
The Lode's own tuning lives in `config.ts` → `LODE`: 2000 base HP (× the wave `hpMult`, and **nothing else** — no per-appearance stacking), 40 contact damage, sprite scale 2.5 (160px), a fixed 16s traverse at ~130 px/s that ignores the wave `speedMult`, a Star dripped every 1.6s, and a death burst of `10 + floor((wave − 15) / 3)` Stars. That window asks ~335 sustained DPS at wave 15 and ~635 at wave 30. Move the whole event with the two knobs `LODE.startWave` and `LODE.everyWaves`. See ADR-0021.
