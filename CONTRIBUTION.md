...
| ![Duelist](public/assets/SpaceShooter/Enemies/fighter3.png) | **Duelist** | Wave 35 | Quick skirmisher that takes up one firing position after another near the top: flies to a spot picked off your position, fires a 3-shot burst along its nose on arrival, dwells, then picks a new spot. Nose always points at you (the first rotated enemy). Never flees, max 2 live. |
...
**Per-kill XP** (`config.ts` → `XP`): Swarmer 2 · Gunner 4 · Asteroid 4/2/1 (large/medium/small) · Mine 4 · Bomber 6 · Warden 14 · SpaceStation 16 · Duelist 18 · Mini-boss 30 · Boss 60 · **Lode 60**. Star pickup: 12.

**Per-kill Score** (`config.ts` → `SCORE`): Swarmer 10 · Gunner 30 · Asteroid 25/12/5 · Mine 25 · Bomber 35 · Warden 100 · SpaceStation 130 · Duelist 140 · Mini-boss 250 · Boss 600 · **Lode 600**.

The Duelist's tuning lives in `config.ts` → `DUELIST`: 130 base HP (× the wave `hpMult`, and **nothing else** — it never flees, so per-appearance stacking would soft-lock a run), 14 bullet damage × a 3-shot burst, sprite scale 2 (64×64 — the Swarmer's footprint), and spots picked 310px from the player inside a 350–620px band, which works out to a real 310–390px gap. Its loop is driven by **arrival, not a clock**: it eases (smoothstep, ~900 px/s average) to a committed spot, fires the moment it lands, dwells 1.6s, then picks the next. Three knobs carry the feel: `dwellSeconds` (the player's window to line up under it — the main difficulty knob after `hp`), `standoffRadius` **together with `bandMaxY`** (the band is what binds at the player's home row, so moving one alone barely moves the Duelist), and `turnRate`. **`turnRate` sits in a window with a wall on each side, both set by the standoff: too high and a committed break cannot be outrun, too low and a gentle drift outruns it too.** At the current 310px that window is ~1.3–3.2 rad/s and it ships at 2.6 — and note the walls close in as the Duelist gets nearer, so re-derive it whenever the distance changes. Spots are chosen in exactly one place (`chooseDuelistSpot`) and then frozen — that is what keeps "it doesn't follow you" true by construction. `maxLive` (2) is enforced at spawn time in `WaveManager`, not at wave composition, because the wave queue is built before anything is live. See ADR-0024.
