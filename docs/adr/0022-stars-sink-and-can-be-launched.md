...
- `STAR.sinkSpeed`, `burstSpeedMin`/`burstSpeedMax`, and `burstDamping` are the knobs. The damping is tuned so a burst's outward drift matches the Kill Burst's footprint at the Lode's sprite scale — retuning the burst speeds without the damping will break that visual match.
